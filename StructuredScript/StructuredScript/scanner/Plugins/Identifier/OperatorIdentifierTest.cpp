...
namespace StructuredScript{
	TEST(OperatorIdentifier, Test){
		...
		Scanner::StringCharacterWell mcw_1("string"), mcw_2("$4518"), mcw_3("operator +"), mcw_4("operator left ++"),
			mcw_5("operator right +++"), mcw_6("operator new"), mcw_7("operator[]ll"), mcw_8("operator left"), mcw_9("operator right &");
		...
		CHECK(opId.get(mcw_3) == Scanner::Token(Scanner::Plugins::BinaryOperatorTokenType::type, "+"));
		mcw_3.commit();
		CHECK(mcw_3.dump() == "");

		id.get(mcw_4);
		CHECK(opId.matches(mcw_4));
		CHECK(opId.get(mcw_4) == Scanner::Token(Scanner::Plugins::LeftUnaryOperatorTokenType::type, "++"));
		mcw_4.commit();
		CHECK(mcw_4.dump() == "");

		id.get(mcw_5);
		CHECK(opId.matches(mcw_5));
		CHECK(opId.get(mcw_5) == Scanner::Token(Scanner::Plugins::RightUnaryOperatorTokenType::type, "++"));
		mcw_5.commit();
		CHECK(mcw_5.dump() == "+");

		id.get(mcw_6);
		CHECK(opId.matches(mcw_6));
		CHECK(opId.get(mcw_6) == Scanner::Token(Scanner::Plugins::BinaryOperatorTokenType::type, "new"));
		mcw_6.commit();
		CHECK(mcw_6.dump() == "");

		id.get(mcw_7);
		CHECK(opId.matches(mcw_7));
		CHECK(opId.get(mcw_7) == Scanner::Token(Scanner::Plugins::BinaryOperatorTokenType::type, "[]"));
		mcw_7.commit();
		CHECK(mcw_7.dump() == "ll");

		id.get(mcw_8);
		CHECK(opId.matches(mcw_8));
		CHECK(opId.get(mcw_8) == Scanner::Token(Scanner::TokenType::TOKEN_TYPE_ERROR, "operator left"));
		mcw_8.commit();
		CHECK(mcw_8.dump() == "");
	}
}